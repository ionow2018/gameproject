...
def load_record(filename, new=-1):
    place = -1
    filename = "data/" + filename
    scores = []
    try:
...
class Shooter(pygame.sprite.Sprite):
    def __init__(self, x, y, image, group, health=30, health_max=30, shoot=5, speed=50, limit=180, accuracy=9):
        ...
        self.x = x
        self.y = y
        self.rect.centerx = x  # - self.hw
        self.rect.centery = y  # - self.hh
        self.health = health
        self.points = health
    ...
    def shoot(self, enemy_x, enemy_y, shooter_number):
...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, x, y, xtarget, ytarget, image, limit=350, power=9,
                 shooter_number=0, speed=200, shooter_radius=15):
        ...
        if self.distance > 0:
            self.dx = (xtarget - x) / self.distance
    ...
    def hit(self, targets):
        ret = False
        if pygame.sprite.spritecollideany(self, targets):
            ret = True
            ...
            if not pause:
                ...
                for i, enemy in enumerate(enemies):
                    ...
                    if (enemy_shoots + i * 15) % 40 == 0:
                        bullet = enemy.shoot(my.x, my.y, i)
